...
public class JwtTokenService {
    ...
    public JwtToken createAccessJwtToken(UserInfo userInfo) {
        ...
        String token = Jwts.builder().setClaims(claims).setIssuer(configurations.getTokenIssuer()).setIssuedAt(Date.from(
                currentTime.atZone(ZoneId.systemDefault()).toInstant())).setExpiration(Date.from(
                        currentTime.plusMinutes(configurations.getTokenExpirationTime()).atZone(ZoneId.systemDefault()).toInstant())).signWith(
    ...
    public JwtToken createRefreshToken(UserInfo userInfo) {
        ...
        String token = Jwts.builder().setClaims(claims).setIssuer(configurations.getTokenIssuer()).setId(
                UUID.randomUUID().toString()).setIssuedAt(Date.from(currentTime.atZone(ZoneId.systemDefault()).toInstant())).setExpiration(Date.from(
                        currentTime.plusMinutes(configurations.getRefreshTokenExpTime()).atZone(ZoneId.systemDefault()).toInstant())).signWith(
                                getSecretKey()).compact();
        return new AccessJwtToken(token);
    }
