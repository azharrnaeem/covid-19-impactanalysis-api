...
public class DataAnalysisService {
	...
	public NewCasesToday getNewCasesToday(Date requestedDate) {
		NewCasesToday newCasesInfo = new NewCasesToday();
		List<Date> keyList = inMemoryParsedData.getSortedKeys();
		requestedDate = null == requestedDate ? CommonUtils.getSystemDateWithoutTime() : requestedDate;
		int dateIndex = Collections.binarySearch(keyList, requestedDate);
		if (dateIndex < 0) {
			newCasesInfo.setDescription(String.format("Right now data is not available for date: %s", CommonUtils.formatDate(Constants.DATE_FORMAT_MM_DD_YY, requestedDate)));
			return newCasesInfo;
		}
	...
	public CountryWiseCases getNewCasesCountrywise(Date requestedDate) {
		...
		requestedDate = null != requestedDate ? requestedDate : CommonUtils.getSystemDateWithoutTime();
		List<Date> keyList = inMemoryParsedData.getSortedKeys();
		Map<String, Long> dataOnRequestedDate = inMemoryParsedData.getDateWiseCasesData().getOrDefault(requestedDate, new ConcurrentHashMap<>());
		if (dataOnRequestedDate.size() == 0) {
			countryWiseCases.setDescription(String.format("Right now data is not available for date: %s", CommonUtils.formatDate(Constants.DATE_FORMAT_MM_DD_YY, requestedDate)));
			return countryWiseCases;
		}
		List<CasesInfo> caseInfoList = dataOnRequestedDate.entrySet().stream().map(entry -> new CasesInfo(entry.getKey(), entry.getValue())).collect(Collectors.toList());
		...
		List<CasesInfo> sortedDataList = caseInfoList.stream().filter(caseInfo -> caseInfo.getCases() >= 0).sorted(Comparator.comparing(CasesInfo::getCases).reversed()).collect(Collectors.toList());
		countryWiseCases.setCountryWiseCasesData(sortedDataList);
		return countryWiseCases;
	}

	public CasesInfo getNewCasesReportedInCountryToday(String country) {
		Optional<List<CasesInfo>> countryWiseDataForToday = Optional.ofNullable(getNewCasesCountrywise(null).getCountryWiseCasesData());
		Optional<CasesInfo> requestCountryData = countryWiseDataForToday.orElse(new ArrayList<CasesInfo>(0)).stream().filter(caseInfo -> country.equalsIgnoreCase(caseInfo.getCountryName())).findAny();
		return requestCountryData.orElse(new CasesInfo(country, 0L));
	}

	public CountryWiseCases getTopNCountriesWithNewCasesToday(int count) {
		CountryWiseCases countryWiseCases = new CountryWiseCases();
		Optional<List<CasesInfo>> countryWiseDataForToday = Optional.ofNullable(getNewCasesCountrywise(null).getCountryWiseCasesData());
		List<CasesInfo> topNCountriesData = countryWiseDataForToday.orElse(new ArrayList<CasesInfo>()).stream().limit(count).collect(Collectors.toList());
	...
	public DatewiseCountryData getCasesDataInCountrySinceDate(String country, Date referenceDate) {
		...
		if (!inMemoryParsedData.getDateWiseCasesData().containsKey(referenceDate) || !inMemoryParsedData.getDateWiseCasesData().get(referenceDate).containsKey(country)) {
